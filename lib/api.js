...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  send(eventName, data) {
    ...
    // Special logging for session.update
    if (eventName === 'session.update') {
      console.log("KDLF: WEBSOCKET SEND session.update - Full event object:", JSON.stringify(event, null, 2));
      console.log("KDLF: WEBSOCKET SEND - input_audio_noise_reduction:", event.session?.input_audio_noise_reduction);
      if (!event.session?.input_audio_noise_reduction) {
        console.log("KDLF: WARNING! input_audio_noise_reduction is MISSING from WebSocket send!");
      }
    }
