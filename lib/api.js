...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  send(eventName, data) {
    ...
    if (eventName === 'session.update') {
      console.log("KDLN: WEBSOCKET SEND session.update - Full event object:", JSON.stringify(event, null, 2));
      console.log("KDLN: WEBSOCKET SEND - input_audio_noise_reduction:", event.session?.input_audio_noise_reduction);
      if (!event.session?.input_audio_noise_reduction) {
        console.log("KDLN: WARNING! input_audio_noise_reduction is MISSING from WebSocket send!");
      }
    }
