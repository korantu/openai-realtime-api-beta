...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  async connect() {
    ...
    console.log("KDLN: connect() - sessionConfig before connection:", JSON.stringify(this.sessionConfig, null, 2));
    ...
    if (!session.input_audio_noise_reduction) {
      console.log("KDLN: connect() - FORCING input_audio_noise_reduction!");
    ...
    console.log("KDLN: connect() - Sending initial session config:", JSON.stringify(session, null, 2));
    console.log("KDLN: connect() - input_audio_noise_reduction:", session.input_audio_noise_reduction);
  ...
  updateSession({
    ...
    console.log("KDLN: updateSession called with input_audio_noise_reduction:", input_audio_noise_reduction);
    console.log("KDLN: Current sessionConfig BEFORE update:", JSON.stringify(this.sessionConfig, null, 2));
    ...
    console.log("KDLN: sessionConfig AFTER update:", JSON.stringify(this.sessionConfig, null, 2));
    ...
    // FORCE input_audio_noise_reduction to be included
    if (!session.input_audio_noise_reduction) {
      console.log("KDLN: FORCING input_audio_noise_reduction to be added!");
      session.input_audio_noise_reduction = { type: "near_field" };
    }
    
    console.log("KDLN: Final session object being sent:", JSON.stringify(session, null, 2));
    console.log("KDLN: input_audio_noise_reduction in final session:", session.input_audio_noise_reduction);
    
    if (this.realtime.isConnected()) {
      console.log("KDLN: SENDING session.update NOW with input_audio_noise_reduction:", session.input_audio_noise_reduction);
      this.realtime.send('session.update', { session });
    } else {
      console.log("KDLN: NOT CONNECTED - session update will be sent on connect");
    }
    return true;
  }
