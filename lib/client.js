 ...
 * @property {{type: string}} [input_audio_noise_reduction]
 */
...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    console.log("KDLN", this.defaultSessionConfig);
    this.sessionConfig = {};
  ...
  async connect() {
    ...
    console.log("KDLF: connect() - sessionConfig before connection:", JSON.stringify(this.sessionConfig, null, 2));
    
    await this.realtime.connect();
    
    // Send initial session configuration immediately after connection
    // This ensures input_audio_noise_reduction is set during session creation
    ...
    // FORCE input_audio_noise_reduction to be included
    if (!session.input_audio_noise_reduction) {
      console.log("KDLF: connect() - FORCING input_audio_noise_reduction!");
      session.input_audio_noise_reduction = { type: "near_field" };
    }
    
    // Wait a small amount of time to ensure the connection is established
    await new Promise(resolve => setTimeout(resolve, 100));
    
    console.log("KDLF: connect() - Sending initial session config:", JSON.stringify(session, null, 2));
    console.log("KDLF: connect() - input_audio_noise_reduction:", session.input_audio_noise_reduction);
  ...
  addTool(definition, handler) {
    ...
    // Don't automatically update session - let the user control when to update
    // this.updateSession();
  ...
  updateSession({
    ...
    console.log("KDLF: updateSession called with input_audio_noise_reduction:", input_audio_noise_reduction);
    console.log("KDLF: Current sessionConfig BEFORE update:", JSON.stringify(this.sessionConfig, null, 2));
    ...
    input_audio_noise_reduction !== void 0 &&
      (this.sessionConfig.input_audio_noise_reduction = input_audio_noise_reduction);
    
    console.log("KDLF: sessionConfig AFTER update:", JSON.stringify(this.sessionConfig, null, 2));
    ...
    // FORCE input_audio_noise_reduction to be included
    if (!session.input_audio_noise_reduction) {
      console.log("KDLF: FORCING input_audio_noise_reduction to be added!");
      session.input_audio_noise_reduction = { type: "near_field" };
    }
    
    console.log("KDLF: Final session object being sent:", JSON.stringify(session, null, 2));
    console.log("KDLF: input_audio_noise_reduction in final session:", session.input_audio_noise_reduction);
    
    if (this.realtime.isConnected()) {
      console.log("KDLF: SENDING session.update NOW with input_audio_noise_reduction:", session.input_audio_noise_reduction);
      this.realtime.send('session.update', { session });
    } else {
      console.log("KDLF: NOT CONNECTED - session update will be sent on connect");
    }
    return true;
  }
